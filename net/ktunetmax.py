...
class UNETR(nn.Module):
    ...
    def __init__(
        ...
        self.encoder1 = nn.Conv3d(1, 32, 3, stride=1,
                                  padding=1)
        # First Layer GrayScale Image , change to input channels to 3 in case of RGB
        self.en1_bn = nn.BatchNorm3d(32)
        self.encoder2 = nn.Conv3d(32, 64, 3, stride=1, padding=1)
        self.en2_bn = nn.BatchNorm3d(64)
        self.encoder3 = nn.Conv3d(64, 128, 3, stride=1, padding=1)
        self.en3_bn = nn.BatchNorm3d(128)

        self.decoder1 = nn.Conv3d(128, 64, 3, stride=1, padding=1)
        self.conv = nn.Conv3d(128, 64, kernel_size=1)
        self.de1_bn = nn.BatchNorm3d(64)
        ...
        self.de3_bn = nn.BatchNorm3d(16)

        self.decoderf1 = nn.Conv3d(128, 64, 3, stride=1, padding=1)
        self.def1_bn = nn.BatchNorm3d(32)
        self.decoderf2 = nn.Conv3d(64, 32, 3, stride=1, padding=1)
        self.def2_bn = nn.BatchNorm3d(16)
        self.decoderf3 = nn.Conv3d(32, 16, 3, stride=1, padding=1)
        self.def3_bn = nn.BatchNorm3d(8)

        self.encoderf1 = nn.Conv3d(1, 32, 3, stride=1, padding=1)
        # First Layer GrayScale Image , change to input channels to 3 in case of RGB
        self.enf1_bn = nn.BatchNorm3d(32)
        self.encoderf2 = nn.Conv3d(32, 64, 3, stride=1, padding=1)
        self.enf2_bn = nn.BatchNorm3d(32)
        self.encoderf3 = nn.Conv3d(64, 128, 3, stride=1, padding=1)
        self.enf3_bn = nn.BatchNorm3d(128)

        self.intere1_1 = nn.Conv3d(32, 32, 3, stride=1, padding=1)
        self.inte1_1bn = nn.BatchNorm3d(32)
        self.intere2_1 = nn.Conv3d(64, 64, 3, stride=1, padding=1)
        self.inte2_1bn = nn.BatchNorm3d(64)
        self.intere3_1 = nn.Conv3d(128, 128, 3, stride=1, padding=1)
        self.inte3_1bn = nn.BatchNorm3d(128)

        self.intere1_2 = nn.Conv3d(32, 32, 3, stride=1, padding=1)
        self.inte1_2bn = nn.BatchNorm3d(32)
        self.intere2_2 = nn.Conv3d(64, 64, 3, stride=1, padding=1)
        self.inte2_2bn = nn.BatchNorm3d(64)
        self.intere3_2 = nn.Conv3d(128, 128, 3, stride=1, padding=1)
        self.inte3_2bn = nn.BatchNorm3d(128)

        self.interd1_1 = nn.Conv3d(64, 64, 3, stride=1, padding=1)
        self.intd1_1bn = nn.BatchNorm3d(64)
        self.interd2_1 = nn.Conv3d(32, 32, 3, stride=1, padding=1)
        self.intd2_1bn = nn.BatchNorm3d(32)
        self.interd3_1 = nn.Conv3d(128, 128, 3, stride=1, padding=1)
        self.intd3_1bn = nn.BatchNorm3d(128)

        self.interd1_2 = nn.Conv3d(64, 64, 3, stride=1, padding=1)
        self.intd1_2bn = nn.BatchNorm3d(64)
        self.interd2_2 = nn.Conv3d(32, 32, 3, stride=1, padding=1)
        self.intd2_2bn = nn.BatchNorm3d(32)
        self.interd3_2 = nn.Conv3d(128, 128, 3, stride=1, padding=1)
        self.intd3_2bn = nn.BatchNorm3d(128)
    ...
    def forward(self, x_in):
        ...
        # print("x", x.size())
        out = F.relu(self.en1_bn(F.max_pool3d(self.encoder1(x_in), 2, 2)))
        out1 = F.relu(self.enf1_bn(F.interpolate(self.encoderf1(x_in), scale_factor=(0.5, 1, 1),
        ...
        out1 = F.relu(self.enf2_bn(F.interpolate(self.encoderf2(out1), scale_factor=(1, 1, 1)
        ...
        out1 = F.relu(self.enf3_bn(F.interpolate(self.encoderf3(out1), scale_factor=(2, 2, 2),
                                                 mode='trilinear', align_corners=False)))

        tmp = out

        out = torch.add(out, F.interpolate(F.relu(self.inte3_1bn(self.intere3_1(out1))),
                                           scale_factor=(0.125, 0.0625, 0.0625), mode='trilinear', align_corners=False))
        out1 = torch.add(out1, F.interpolate(F.relu(self.inte3_2bn(self.intere3_2(tmp))),
        ...
        tmp = torch.cat((out, enc), dim=1)
        temp = F.relu(self.de1_bn(self.conv(tmp)))
        out1 = F.relu(self.def1_bn(F.max_pool3d(self.decoderf1(out1), 2, 2)))  # Ki-NET

        out = torch.add(temp, F.interpolate(F.relu(self.intd1_1bn(self.interd1_1(out1))),
                                            scale_factor=(0.5, 0.25, 0.25), mode='trilinear', align_corners=False))
        out1 = torch.add(out1, F.interpolate(F.relu(self.intd1_2bn(self.interd1_2(temp))),
        ...
        out = F.relu(self.de2_bn(F.interpolate(self.decoder2(out), scale_factor=(2, 2, 2),
                                               mode='trilinear', align_corners=False)))
        ...
        out = F.relu(self.de3_bn(F.interpolate(self.decoder3(out), scale_factor=(1, 2, 2),
                                               mode='trilinear', align_corners=False)))
