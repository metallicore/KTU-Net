...
class InitConv(nn.Module):
    def __init__(self, in_channels=4, out_channels=16, norm='gn', dropout=0.2):
        super(InitConv, self).__init__()

        self.conv = nn.Conv3d(in_channels, out_channels, kernel_size=3, padding=1)
        self.bn = normalization(out_channels, norm=norm)
...
if __name__ == '__main__':
    ...
    net = Unet(in_channels=4, base_channels=16, num_classes=4)
    y = net(x)
