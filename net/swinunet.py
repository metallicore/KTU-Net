...
class BasicLayer_up(nn.Module):
    ...
    def __init__(self,
        ...
        if upsample is not None:
            self.upsample = Patch_Expanding(input_resolution, dim=dim, norm_layer=norm_layer)
        else:
            self.upsample = None
...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        ...
        assert S == self.img_size[0] and H == self.img_size[1] and W == self.img_size[2], \
            f"Input image size ({S}*{H}*{W}) doesn't match model " \
...
class SwinTransformerSys(nn.Module):
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
        ...
        for i_layer in range(self.num_layers):
            ...
            if i_layer == 0:
                ...
                layer_up = BasicLayer_up(dim=int(embed_dim * 2 ** (self.num_layers - 1 - i_layer)),
                                         ...
                                         norm_layer=norm_layer,
                                         upsample=Patch_Expanding if (i_layer < self.num_layers - 1) else None,
                                         use_checkpoint=use_checkpoint)
        ...
        if self.final_upsample == "expand_first":
            ...
            self.up = final_patch_expanding(dim=embed_dim, num_class=2, patch_size=patch_size)
